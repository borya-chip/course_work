...
bool FileManager::saveToBinary(const InventoryManager& inventory, const std::string& filename) {
    try {
...
bool FileManager::loadFromBinary(InventoryManager& inventory, const std::string& filename) {
    try {
        QFile file(QString::fromStdString(filename));
...
bool FileManager::exportReportToText(const InventoryManager& inventory, const std::string& filename) {
    try {
        ...
        for (const auto& product : products) {
            if (product) {
                ...
                file << "Status: " << (product->isExpired() ? "EXPIRED" : "Active") << "\n";
                file << "---\n\n";
            }
        }
...
bool FileManager::exportWriteOffHistoryToText(const InventoryManager& inventory, const std::string& filename) {
    try {
        std::ofstream file(filename);
        if (!file.is_open()) {
            return false;
