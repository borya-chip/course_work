#ifndef PRODUCTREPOSITORY_CPP
#define PRODUCTREPOSITORY_CPP
...
template<typename T>
void ProductRepository<T>::sortByExpirationDate() {
    std::sort(products.begin(), products.end(),
        [](const std::shared_ptr<T>& a, const std::shared_ptr<T>& b) {
            if (auto* pa = dynamic_cast<Product*>(a.get())) {
                if (auto* pb = dynamic_cast<Product*>(b.get())) {
                    return pa->getExpirationDate() < pb->getExpirationDate();
                }
            }
            return false;
        });
