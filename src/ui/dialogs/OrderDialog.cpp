...
OrderDialog::OrderDialog(const std::vector<Product>& availableProducts, QWidget* parent)
...
void OrderDialog::setupUI() {
    ...
    // Add Product button - moved up
    QHBoxLayout* addProductLayout = new QHBoxLayout();
    addProductButton = new QPushButton("Add Product", this);
    addProductButton->setStyleSheet("background-color: #4CAF50; color: white; padding: 10px 20px; font-weight: bold; border-radius: 5px;");
    ...
    createOrderButton->setStyleSheet("background-color: #4CAF50; color: white; padding: 10px 20px; font-weight: bold; border-radius: 5px;");
    cancelButton = new QPushButton("Cancel", this);
    cancelButton->setStyleSheet("background-color: #757575; color: white; padding: 10px 20px; font-weight: bold; border-radius: 5px;");
    ...
    connect(retailRadioButton, &QRadioButton::toggled, this, &OrderDialog::onOrderTypeChanged);
    connect(wholesaleRadioButton, &QRadioButton::toggled, this, &OrderDialog::onOrderTypeChanged);
    connect(addProductButton, &QPushButton::clicked, this, &OrderDialog::onAddProductClicked);
    connect(createOrderButton, &QPushButton::clicked, this, &OrderDialog::validateAndAccept);
...
QGroupBox* OrderDialog::createOrderTypeGroupBox() {
    ...
    orderTypeGroup->addButton(retailRadioButton, static_cast<int>(OrderType::RETAIL));
    orderTypeGroup->addButton(wholesaleRadioButton, static_cast<int>(OrderType::WHOLESALE));
...
QGroupBox* OrderDialog::createCartGroupBox() {
    ...
    cartTable->setHorizontalHeaderLabels({"Product", "Quantity", "Price", "Amount", "Discount", "Total", "Actions"});
    ...
    cartTable->horizontalHeader()->setSectionResizeMode(6, QHeaderView::ResizeToContents); // Actions column
...
QGroupBox* OrderDialog::createTotalsGroupBox() {
    ...
    finalTotalLabel->setStyleSheet("font-weight: bold; font-size: 16pt; color: #2196F3;");
...
void OrderDialog::onOrderTypeChanged() {
    OrderType type = retailRadioButton->isChecked() ? OrderType::RETAIL : OrderType::WHOLESALE;
...
void OrderDialog::onAddProductClicked() {
    ...
    // Create a filtered list of products with updated quantities (considering items already in cart)
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        for (const auto& item : currentOrder.getItems()) {
            if (item.productId == product.getId()) {
                ...
                auto it = std::find_if(availableProducts.begin(), availableProducts.end(),
                    [&product](const Product& p) { return p.getId() == product.getId(); });
                if (it != availableProducts.end() && totalQty > it->getQuantity()) {
                    QMessageBox::warning(this, "Error", 
                        QString("Total quantity exceeds available stock! Available: %1, Already in cart: %2")
                        .arg(it->getQuantity()).arg(item.quantity));
                    return;
                }
                // Update quantity
...
void OrderDialog::onQuantityChanged(int row, int quantity) {
    ...
    if (idItem) {
        ...
        // Find product in available products to check stock
        auto it = std::find_if(availableProducts.begin(), availableProducts.end(),
        ...
        if (it != availableProducts.end()) {
            ...
            if (quantity > it->getQuantity()) {
                QMessageBox::warning(this, "Error", 
                    QString("Insufficient stock! Available: %1").arg(it->getQuantity()));
...
void OrderDialog::refreshCartTable() {
    ...
    for (const auto& item : items) {
        ...
        // Product name (store ID in UserRole)
        ...
        // Quantity (editable) - set max to available stock
        QSpinBox* qtySpinBox = new QSpinBox();
        qtySpinBox->setMinimum(1);
        // Find product in available products to get max stock
        int maxStock = 9999;
        auto it = std::find_if(availableProducts.begin(), availableProducts.end(),
        ...
        connect(qtySpinBox, QOverload<int>::of(&QSpinBox::valueChanged),
                [this, row](int value) { 
                    onQuantityChanged(row, value);
                });
        cartTable->setCellWidget(row, 1, qtySpinBox);
        
        // Price
        cartTable->setItem(row, 2, new QTableWidgetItem(QString("$%1").arg(item.unitPrice, 0, 'f', 2)));
        
        // Amount (before discount)
        double amount = item.unitPrice * item.quantity;
        cartTable->setItem(row, 3, new QTableWidgetItem(QString("$%1").arg(amount, 0, 'f', 2)));
        
        // Discount
        double discount = DiscountCalculator::calculateDiscount(isWholesale, item.quantity);
        QTableWidgetItem* discountItem = new QTableWidgetItem(QString("%1%").arg(discount, 0, 'f', 1));
        if (discount > 0) {
        ...
        cartTable->setItem(row, 5, new QTableWidgetItem(QString("$%1").arg(item.totalPrice, 0, 'f', 2)));
    ...
    // Ensure columns stretch to fill window width
    cartTable->horizontalHeader()->setStretchLastSection(false);
    cartTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    cartTable->horizontalHeader()->setSectionResizeMode(6, QHeaderView::ResizeToContents); // Actions column
}
