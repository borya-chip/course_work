...
ReportDialog::ReportDialog(const InventoryManager& inventory, QWidget* parent)
...
void ReportDialog::generateInventoryReport() {
    ...
    auto expired = inventoryManager.getExpiredProducts();
    report += QString("Expired Products: %1\n\n").arg(static_cast<int>(expired.size()));
    ...
    for (const auto& product : products) {
        if (product) {
            ...
            report += QString("Status: %1\n")
                      .arg(product->isExpired() ? "EXPIRED" : "Active");
            report += "---\n\n";
        }
    }
...
void ReportDialog::generateWriteOffReport() {
    ...
    report += QString("Total Write-offs: %1\n\n").arg(static_cast<int>(writeOffHistory.size()));
